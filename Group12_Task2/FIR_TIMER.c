...
float filter1(float Input_Data)
{
   ...
   // to be completed
    ...
    //compute multiplication and summation
    //use circular buffer to hold input signal
    filtbuff1[0]=Input_Data; //push new sample (Input_Data) into the circular buffer
    sum = firCoeffs[0]*filtbuff1[0];
    for(i=FIR_FILTER_LENGTH-1; i>=1; i--){
        sum=sum+firCoeffs[i]*filtbuff1[i];//multiplication
        filtbuff1[i]=filtbuff1[i-1];//shift operation
    }
